...
function checkNpmRegistry() {
    const userRegistry = execSync('npm config get registry', { encoding: 'utf-8' });

    if (userRegistry.includes('https://registry.yarnpkg.com/')) {
        printErrorAndExit(`Release failed, please use ${color.blue}npm run release`);
    }
}
...
function release() {
    ...
    const updatedStdout = spawnSync(lernaCli, ['changed'], { encoding: 'utf-8' });
    ...
    spawnSync(lernaCli, ['version', '--exact', '--no-commit-hooks', '--no-git-tag-version', '--no-push']);
